...
open class AnalogJoystick: SKNode {
    ...
    var yAxisBlocked = false
    var xAxisBlocked = false
    ...
    @objc func listen() {
        ...
        if tracking {
            AnalogJoystick.logCount += 1
            if(AnalogJoystick.logCount % 10 == 0) {
    ...
    open override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch: AnyObject in touches {
            ...
            let xNeedPosition = realDistantion <= maxDistantion ? (xAxisBlocked ? stick.position.x : location.x) : (xAxisBlocked ? stick.position.x : location.x / realDistantion * maxDistantion)
            let yNeedPosition = realDistantion <= maxDistantion ? (yAxisBlocked ? stick.position.y : location.y) : (yAxisBlocked ? stick.position.y : location.y / realDistantion * maxDistantion)
            let needPosition = CGPoint(x: xNeedPosition, y: yNeedPosition)
